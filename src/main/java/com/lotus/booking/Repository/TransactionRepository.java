...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query(value="SELECT SUM(t.amount) as amount, SUM(t.count) as count, t.date FROM Transactions t WHERE t.user_id =?1 AND t.date BETWEEN ?2 AND ?3 GROUP BY t.date ORDER by t.date ASC;",nativeQuery = true)
